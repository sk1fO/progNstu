...
type Queue struct {
	list *List // Указатель на список, используемый для хранения элементов очереди
}

// создает и возвращает указатель на новую пустую очередь
func NewQueue() *Queue {
	return &Queue{list: NewList()} // Создаем новую очередь с пустым списком
}

// добавление элемента в конец очереди
func (q *Queue) Push(value interface{}) {
	q.list.AddToTail(value) // Добавляем элемент в конец списка
}
